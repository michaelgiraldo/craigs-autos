...
function leadEventDefinition(args) {
  return Object.freeze({
    ...
    browserSignal: Boolean(args.browserSignal),
...
export const LEAD_BROWSER_SIGNAL_EVENT_NAMES = Object.freeze(
  LEAD_EVENT_NAMES.filter((eventName) => LEAD_EVENT_DEFINITIONS[eventName]?.browserSignal),
);
...
export function isLeadBrowserSignalEventName(value) {
  const definition = typeof value === 'string' ? getLeadEventDefinition(value) : null;
  return Boolean(definition?.browserSignal);
}
