...
test('public lead interaction endpoint only accepts journey interaction events', () => {
  assert.deepEqual(LEAD_INTERACTION_EVENT_NAMES, [
  ...
  for (const eventName of LEAD_EVENT_NAMES) {
    ...
    assert.equal(isLeadInteractionEventName(eventName), definition?.browserInteraction ?? false);
  }
});
