...
type ChatSessionCreateRequest = z.infer<typeof sessionPayloadSchema>;
...
export function createChatSessionCreateHandler(deps: SessionHandlerDeps) {
  return async (event: LambdaEvent): Promise<LambdaResult> => {
    ...
    let payload: ChatSessionCreateRequest = {};
    try {
...
export const handler = createChatSessionCreateHandler({
