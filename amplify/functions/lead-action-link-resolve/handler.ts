...
const leadActionLinksEnvSchema = z.object({
  LEAD_ACTION_LINKS_TABLE_NAME: z.string().trim().min(1),
});
...
const parsedLeadActionLinksEnv = leadActionLinksEnvSchema.safeParse(process.env);
const runtimeTokenTableName = parsedLeadActionLinksEnv.success
  ? parsedLeadActionLinksEnv.data.LEAD_ACTION_LINKS_TABLE_NAME
  : '';
