...
import { renderConversionFeedbackDetail } from './render-conversion-feedback';
import { renderFollowupWorkTable } from './render-followup-work';
...
function renderLeadRecordRow(
  ...
  const feedbackDetail = renderConversionFeedbackDetail(item);
  if (feedbackDetail) {
    feedbackCell.appendChild(feedbackDetail);
  }
...
function renderDashboard(
  ...
  const followupHeading = createTextElement('h2', 'Follow-Up Work');
  followupHeading.style.margin = '28px 0 4px';
  followupHeading.style.fontSize = '1.1rem';
  wrapper.appendChild(followupHeading);
  wrapper.appendChild(
    createTextElement(
      'p',
      'Operational queue for queued, processing, and failed first-response work.',
      'muted',
    ),
  );
  wrapper.appendChild(renderFollowupWorkTable(state, actions));
