...
- `amplify/functions/chat-session-create` (Lambda): creates short-lived OpenAI session secrets for the browser.
- `amplify/functions/chat-handoff-promote` (Lambda): evaluates transcript, persists the lead journey, sends the shop notification email, and runs QUO SMS when configured.
- `amplify/functions/lead-action-link-resolve` (Lambda): resolves short-lived token links used by
  message handoff actions in lead notification emails (SMS entry).
- `DynamoDB (ChatHandoffPromoteDedupeTable)`: enforces one completed handoff per thread and tracks lease/error state.
- `DynamoDB (LeadJourneys/LeadRecords/LeadContacts/LeadJourneyEvents)`: stores the lead journey and qualification state.
- `DynamoDB (LeadActionLinkResolveTokenTable)`: stores message draft links with TTL.
...
- Handoff dedupe (`DynamoDB`):
  ...
  - Current TTL is configured in `amplify/functions/chat-handoff-promote/handler.ts`.
- Lead journey data (`DynamoDB`):
  - Journey/contact/event/record tables are the operational lead system.
- Lead action links (`DynamoDB`):
  - `LEAD_ACTION_LINK_TTL_DAYS` in `runtime.ts`.
...
Customer chats in ChatKit
  └─> Frontend tracks activity and interaction
      ├─> After 5 min inactivity while open -> POST /chat-handoffs { reason: "idle" }
      ├─> On tab hide/visibility change -> POST /chat-handoffs { reason: "pagehide" }
      └─> On panel close -> POST /chat-handoffs { reason: "chat_closed" }
             |
             v
...
- Backend lead processing and safety gates
  - `Website/amplify/functions/chat-handoff-promote/handler.ts`
...
- Attachment preview + inline embedding
  - `Website/amplify/functions/chat-handoff-promote/email-delivery.ts`
  - `Website/amplify/functions/chat-handoff-promote/attachments.ts`
  - `Website/amplify/functions/chat-handoff-promote/email-mime.ts`
...
- OpenAI logs and DynamoDB dedupe record are still the main source of truth
  (`cthr_...`).
- Raw email delivery issues will show up in SES send logs and CloudWatch (`chat-handoff-promote`).
